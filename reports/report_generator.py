...
@page {
    ...
    /* Footer */
    @bottom-left {
        content: "Generated by: {version}"; font-size: 8pt;
...
def _build_cover_page(case_info, report_id, now_str, tool_version):
    ...
    <div class="cover-page">
        ...
        <table class="case-info">
            ...
            <tr><th>Diagnostic Engine</th><td>{_esc(tool_version)}</td></tr>
        </table>
...
# ── V2.0 Report Section Builders ─────────────────────────────────────────────

def _build_v2_executive_summary(investigation_summary):
    """Build the v1.0 Executive Summary page with confidence score."""
    if not investigation_summary:
        return ''

    score = investigation_summary.get('confidence_score', 0)
    severity = investigation_summary.get('confidence_severity', 'INFO')
    total = investigation_summary.get('total_findings', 0)
    high = investigation_summary.get('critical_findings', 0) + investigation_summary.get('high_findings', 0)
    medium = investigation_summary.get('medium_findings', 0)
    af_count = investigation_summary.get('anti_forensics_count', 0)
    chain_links = investigation_summary.get('attack_chain_links', 0)

    sev_class = 'sev-high' if severity in ('HIGH', 'CRITICAL') else ('sev-med' if severity == 'MEDIUM' else 'sev-low')

    # Breakdown rows
    breakdown_rows = ''
    for item in investigation_summary.get('confidence_breakdown', []):
        breakdown_rows += f"""
            <tr>
                <td>{_esc(item.get('title', ''))}</td>
                <td>+{item.get('points', 0)}</td>
            </tr>"""

    if not breakdown_rows:
        breakdown_rows = '<tr><td colspan="2" style="text-align: center;">No contributing evidence.</td></tr>'

    return f"""
    <div class="content-section">
        <h2 class="section-heading">Executive Summary &mdash; Endpoint Risk Assessment</h2>

        <table class="metrics-grid">
            <tr>
                <th>Compromise Confidence</th>
                <th>Risk Severity</th>
                <th>Investigation Findings</th>
                <th>Anti-Forensics Alerts</th>
            </tr>
            <tr>
                <td>{score} / 100</td>
                <td><span class="{sev_class}">{severity}</span></td>
                <td>{total} ({high} High)</td>
                <td>{af_count}</td>
            </tr>
        </table>

        <h2 class="section-heading">Confidence Score Breakdown</h2>
        <table class="data-table">
            <tr>
                <th style="width: 70%;">Evidence Factor</th>
...
def _build_v2_findings_section(investigation_summary):
    """Build the v1.0 Correlated Findings section."""
    if not investigation_summary:
        return ''
    ...
    for i, f in enumerate(findings, 1):
        ...
        sources = ', '.join(f.get('evidence_sources', [])) or 'N/A'
        rec = _esc(f.get('recommendation', ''), 120)
        rows_html += f"""
            <tr>
                <td>#{i:03d}</td>
                <td>{_severity_html(sev)}</td>
                <td>{_esc(f.get('title', ''), 40)}</td>
                <td>{sources}</td>
                <td>{rec}</td>
            </tr>"""
    ...
    return f"""
    <div class="content-section">
        <h2 class="section-heading">Evidence Correlation &mdash; Investigation Findings</h2>
        <table class="data-table">
            <tr>
                <th style="width: 6%;">#</th>
                <th style="width: 10%;">Severity</th>
                <th style="width: 24%;">Finding</th>
                <th style="width: 20%;">Supporting Evidence</th>
...
def _build_v2_antiforensics_section(investigation_summary):
    """Build the v1.0 Anti-Forensics Detection section."""
    if not investigation_summary:
        return ''

    alerts = investigation_summary.get('anti_forensics_alerts', [])

    if not alerts:
        content = """
            <tr>
                <td colspan="2" style="text-align: center; color: #2e8b57; font-family: Arial, sans-serif;">
                    No anti-forensics indicators detected &mdash; evidence integrity appears intact.
                </td>
            </tr>"""
    else:
        content = ''
        for af in alerts:
            content += f"""
                <tr>
                    <td>{_esc(af.get('alert_id', ''))}</td>
                    <td>{_esc(af.get('description', ''), 120)}</td>
                </tr>"""

    return f"""
    <div class="content-section">
        <h2 class="section-heading">Anti-Forensics Detection Analysis</h2>
        <table class="data-table">
            <tr>
...
def _build_v2_attack_chain_section(investigation_summary):
    """Build the v1.0 Attack Chain Reconstruction section."""
    if not investigation_summary:
        return ''

    chains = investigation_summary.get('attack_chain', [])
    findings = investigation_summary.get('findings', [])

    # Build a lookup for finding titles
    title_map = {f.get('finding_id', ''): f.get('title', '?') for f in findings}

    if not chains:
        return f"""
    <div class="content-section">
        <h2 class="section-heading">Attack Chain Reconstruction</h2>
        <p style="text-align: center; color: #2e8b57; font-family: Arial, sans-serif;">
            Insufficient correlated findings to reconstruct an attack chain.
        </p>
    </div>
        """
    ...
    for c in chains:
        ...
        nid = c.get('next_finding_id', '')
        rows_html += f"""
            <tr>
                <td>{_esc(title_map.get(fid, fid))}</td>
                <td style="text-align: center; font-weight: bold;">&rarr; {_esc(c.get('relationship', ''))}</td>
                <td>{_esc(title_map.get(nid, nid))}</td>
            </tr>"""
...
def generate_pdf_report(case_info, timeline_data, output_path, db_manager=None,
    ...
    # Determine the Tool Version dynamically based on the current platform
    sys_name = platform.system()
    if sys_name == 'Darwin':
        tool_version = "TriageHound v2.0"
    elif sys_name == 'Linux':
        tool_version = "TriageHound v3.0"
    else:
        tool_version = "TriageHound v1.0"
        
    # Inject dynamic CSS version
    dynamic_css = REPORT_CSS.replace("{version}", tool_version)
...
<body>
    ...
    <!-- COVER PAGE -->
    {_build_cover_page(case_info, report_id, now_str, tool_version)}

    <!-- V2.0: EXECUTIVE SUMMARY & RISK ASSESSMENT -->
    {_build_v2_executive_summary(investigation_summary)}

    <!-- V2.0: CORRELATED FINDINGS -->
    {_build_v2_findings_section(investigation_summary)}

    <!-- V2.0: ANTI-FORENSICS -->
    {_build_v2_antiforensics_section(investigation_summary)}

    <!-- V2.0: ATTACK CHAIN -->
    {_build_v2_attack_chain_section(investigation_summary)}
